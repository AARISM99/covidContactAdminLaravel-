    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $table->editColumn('covid', function ($row) {
                // return $row->covid == 1 ? `<b style="color: red;">positif</b>` : `<b style="color: green;">negatif</b>`;
                // return '<span ' . ($row->covid == 1 ? 'positif' : 'negatif') . '/span>';
                return '<input type="checkbox" disabled ' . ($row->covid == 1 ? 'checked' : null) . '>';
            });
            $table->editColumn('risk', function ($row) {
                // return $row->risk == 1 ? `<b style="color: red;">danger</b>` : `<b style="color: green;">tranquille</b>`;
                // return '<b ' . ($row->risk == 1 ? 'danger' : 'tranquille') . '/b>';
                return '<input type="checkbox" disabled ' . ($row->risk == 1 ? 'checked' : null) . '>';
            });
